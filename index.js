...
function customAssign(...objects) {
  ...
  // Пустий об'єкт, в який будемо об'єднувати властивості
  const combinedObject = {};

  // Об'єднуємо властивості об'єктів
  for (const obj of objects) {
    for (const key in obj) {
      if (obj.hasOwnProperty(key)) {
        // Перетворюємо числові значення на рядки та замінюємо пробіли на підкреслення
        combinedObject[key] = String(obj[key]);
      }
    }
  }
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const FreezeObject = Object.assign(obj1, obj2 );
  Object.freeze(FreezeObject);
  if(Object.isFrozen(FreezeObject) === true){
  return FreezeObject
  } else 
  return ("повідомлення про помилку, якщо обєкт не було заморожено");
...
function checkAndPreventExtensions(obj) {
  if(typeof obj !== "object") {
    return false;
  }else if (Object.isSealed(obj) === true) {
   Object.seal(obj)
   return true;
  }else if (Object.isSealed(obj) === false) {
    return false;  
  } 
}
...
function getObjectValuesSum(obj) {
  if (typeof obj !== "object") {
   ...
   let sum = values.reduce(function (acc, currentValue) {
